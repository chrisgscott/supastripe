...
export default function ConfirmationStep({
  ...
  console.log("ConfirmationStep: Initial render with props:", {
    initialPlanDetails,
    paymentIntent,
  });
  ...
  useEffect(() => {
    console.log("ConfirmationStep: useEffect triggered", {
      initialPlanDetails,
      paymentIntent,
    });
    ...
    const attemptFetch = async () => {
      try {
        const response = await fetch(
          `/api/handle-payment-confirmation?payment_intent=${paymentIntent}`
        );
        ...
        if (data.success) {
          console.log(
            "ConfirmationStep: Setting plan details and loading status"
          );
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Email Sent",
          description:
            "Payment plan details have been sent to the customer's email.",
        });
        setEmailSent(true);
      } else {
        toast({
  ...
  console.log("ConfirmationStep: Rendering with state:", {
    planDetails,
    error,
    loadingStatus,
  });
          ...
          <div className="flex justify-between items-start">
            <div className="text-left">
              <h3 className="font-semibold text-lg">
                {planDetails.businessDetails?.name}
              </h3>
              <p className="text-sm text-muted-foreground">
                {planDetails.businessDetails?.supportPhone}
              </p>
              <p className="text-sm text-muted-foreground">
                {planDetails.businessDetails?.supportEmail}
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-sm text-muted-foreground">
                Plan ID: {planDetails.paymentPlanId}
              </p>
              <p className="text-sm text-muted-foreground">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 gap-8 pt-4">
            ...
            <div className="text-right">
              ...
              <p>
                Total Amount:{" "}
                {formatCurrency(Money.fromCents(planDetails.totalAmount))}
              </p>
              <p className="text-sm text-muted-foreground">
                {planDetails.numberOfPayments} {planDetails.paymentInterval}{" "}
                payments
              </p>
            </div>
          </div>
          ...
          <div className="bg-muted p-4 rounded-lg text-sm">
            ...
            <p>
              Future payments will be automatically processed using the card
              ending in {planDetails.paymentMethod?.last4}. Payments will be
        ...
        <CardFooter className="flex justify-between border-t pt-6 card-actions">
          <div className="flex gap-2">
            <TooltipProvider>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href={`/plan/${planDetails.paymentPlanId}`}>
