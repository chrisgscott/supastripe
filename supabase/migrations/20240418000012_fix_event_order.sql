-- Fix handle_payment_confirmation function to migrate customer before publishing events
...
BEGIN
    ...
    -- Migrate the data (this will create the customer record in customers table)
    ...
    -- Get the new customer_id that was created during migration
    SELECT c.id INTO v_customer_id
    FROM customers c
    JOIN payment_plans pp ON pp.customer_id = c.id
    WHERE pp.id = v_migrated_plan_id;

    IF v_customer_id IS NULL THEN
        RAISE NOTICE 'Failed to find migrated customer';
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Failed to find migrated customer'
        );
    END IF;

    -- Now publish the events with the new customer_id
